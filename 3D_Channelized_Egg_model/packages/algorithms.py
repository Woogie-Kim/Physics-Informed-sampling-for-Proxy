...
class Meta_heuristic:
    def __init__(self,
                 ...
                 setting):
        self.args = args
        if len(perms) == args.num_of_x * args.num_of_y * args.num_of_z:
            self.perms = [perms]
        else:
            self.perms = perms
    ...
    def evaluate(self, positions, neural_model=None):
        ...
        if neural_model:
            for k in self.perms:
                if neural_model.model_name in ['CNN', 'CNN_3D', 'ResNet']:
            ...
            for k in self.perms:
                ...
                if self.setting['Parallel']:
                    ...
                    for file in listdir(position.simulation_directory):
                        if (not 'bat' in file) and (not 'txt' in file) and (not 'DATA' in file) and (
                        not 'F00' in file) and (not 'RSM' in file):
                            remove(join(position.simulation_directory, file))
...
class PSO(Meta_heuristic):
    ...
    def update(self, positions):
        ...
        for particle_idx, position in enumerate(positions_next_):
            ...
            for w in position.wells:
                w._cut_boundaries()
            positions_for_next.append(position)
            # update violation
            position.violation = position.violate(position.wells, True)
        for pos in positions_for_next:
            for w in pos.wells:
                w._cut_boundaries()
        self.positions_all.append(positions_for_next)
        for particle_idx in range(self.setting['Number of particles']):
            self.positions_particle[particle_idx].append(positions_for_next[particle_idx])
    ...
    def _cal_velocity_and_update(self, position, particle_idx):
        ...
        for idx, (well, well_p, well_g) in enumerate(zip(position.wells, pbest_position[particle_idx].wells, gbest_position.wells)):
