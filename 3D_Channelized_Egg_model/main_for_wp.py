...
def main():
    ...
    fix_seed(seed)
    if args.use_proxy:
        print('\nWell placement optimization using Proxy model - 3D Benchmark Egg model')
        print('=' * 82)
    # sampling section #################################################################################################
...
def optimization_section(args, s_setting, opt_setting, seed=0, sample=[], proxy=None, name=None):
    ...
    if name:
        with open(os.path.join(args.cached_dir, f'./WPO_{name}_{seed}_{id}_egg.pkl'), 'wb') as f:
            pickle.dump(WPO, f)
...
def visualization_for_sampling_results(samples, name, id):
    ...
    if (name in ['2stage', 'physics']):
        ...
        NPV_histogram(fitness, seperated=split, bins=np.arange(0, 12e2, 1e2), is_2stage=True, fname=f'hist_{name}')
    else:
        NPV_histogram(fitness, bins=np.arange(0, 12e2, 1e2), is_2stage=False, fname=f'hist_{name}')
...
def visualization_for_proxy_performance(proxy, name, id):
    ...
    get_regression(true, pred, min_=0, max_=1100, fname=f'{args.figure_saved_dir}/proxy_{name}_{id}')


def visualization_for_optmization_results(WPO, name, seed, id):
    plt.rcParams['figure.dpi'] = 300
    wellplacement(WPO.algorithm.gbest[-1], filename=f'wellplacement_{name}_{seed}_{id}_egg')
    draw_relative_error(WPO, f'{name}_{seed}')
    draw_graph(WPO.fits_true['gbest'], WPO.fits_proxy['gbest'], f'Graph_{name}_{seed}_{id}_egg')


def recall_optimization_results(name, seed, id):
    with open(os.path.join(args.cached_dir, f'./WPO_{name}_{seed}_{id}_egg.pkl'), 'rb') as f:
        WPO = pickle.load(f)
    visualization_for_optmization_results(WPO, name, seed, id)
