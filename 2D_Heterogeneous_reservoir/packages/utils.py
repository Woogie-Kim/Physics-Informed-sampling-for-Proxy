...
def draw_qualitymap(qualitys, fname=None):
    ...
    img = ax1.imshow(prod.reshape(60, -1), cmap='jet')
    ...
    img = ax2.imshow(inj.reshape(60, -1), cmap='jet')
...
def draw_samples(samples, what=1, fname=None):
    ...
    loc_p = np.array(list(collections.Counter(idx[zs==what]).keys()))-1
    count_p = list(collections.Counter(idx[zs==what]).values())


    a = np.zeros(args.num_of_x * args.num_of_y)
    a[loc_p] = count_p
    a[a==0] = np.nan
    plt.rcParams['figure.figsize'] = (5, 4)
...
def draw_perm(perm, islog=True, view=True,fname=None):
    p = perm.reshape(args.num_of_x, -1)
    if islog:
        p = np.log(p)
    plt.rcParams['figure.figsize'] = (4,4)
    # plt.rcParams['figure.dpi'] = 150
    plt.rcParams['font.family'] = typo
    plt.imshow(p, cmap=parula_map)
    plt.xlabel('X')
    plt.ylabel('Y')
    plt.xlim([0, args.num_of_x])
    plt.ylim([args.num_of_y, 0])
...
#         pos = [pos]
#         ms = 10
#     draw_perm(pos[0].perm, view=False)
...
def get_CDF(positions, quantile, view=False, fname=None):
    ...
    nwell = np.zeros(args.num_of_max_well - args.num_of_min_well + 1)
    nloc = {label: np.zeros(ngrid) for label in well_type}

    # fitness
    fwell = np.zeros(args.num_of_max_well - args.num_of_min_well + 1)
    ...
    for pos in positions:
        if pos.fit >= th:
            ...
            nwell[tmp_nwell - args.num_of_min_well] += 1
            fwell[tmp_nwell - args.num_of_min_well] += pos.fit
    ...
    # for label in well_type:
    #     pdf = (q_npv[label] / np.sum(q_npv[label]))
    #     eps = (1-sum(pdf))
    #     pdf[np.argmax(pdf)] += eps
    #     pdf_npv[label] = pdf
...
def quality(permeability, islog=False, smooth=False, fname = None, projection = '3d',view=False):
    ...
    x, y = np.where(q.reshape(-1,60) == q.reshape(-1,60))
    if view:
        plt.style.use('default')
...
def SLquality(perm, samples, setting):
    ...
    for k, sample in enumerate(samples):
        ...
        tofp[tofp == 0] = args.max_tof
        tofp_map = tofp.reshape(sample.num_of_x, -1)
        tofp_map[y[t == 1] - 1, x[t == 1] - 1] = 0
    ...
    quality['P'] = np.zeros(len(perm))
    quality['I'] = np.zeros(len(perm))
...
def ViewParticles(Opt, filename, other=True, n_components=3, lag=20, Npv_map=True):
    typo = 'Times New Roman'
    ...
    for i in iter:
        vector_list = []
        NPV = []
        for pbest in Opt.algorithm.pbest[i]:
...
def namer(setting, id, origin):
    if (setting['Method'] in ['2stage', 'physics']):
        return f"wo_{origin}_{setting['Method']}_{id}.pkl"
    else:
        return f"wo_{setting['Number of Samples']}_{setting['Method']}_{id}.pkl"
